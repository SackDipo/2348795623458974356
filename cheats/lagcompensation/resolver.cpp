...
void resolver::resolve_yaw()
{
	...
	if (speed <= 0.1f)
	{
		if (player_record->layers[12].m_flWeight == 0.0 && player_record->layers[12].m_flCycle == 60.0)
		{
			side[player->EntIndex()] = 2 * (math::normalize_diff(player->m_angEyeAngles().y, player_record->abs_angles.y) <= 60.0) - 1;
		}
	}
	else
	{
		const float f_delta = abs(player_record->layers[12].m_flPlaybackRate - player_record->left_layers[12].m_flPlaybackRate);
		const float s_delta = abs(player_record->layers[12].m_flPlaybackRate - player_record->center_layers[12].m_flPlaybackRate);
		const float t_delta = abs(player_record->layers[12].m_flPlaybackRate - player_record->right_layers[12].m_flPlaybackRate);
...
bool get_player_update_animations = true;

float get_clientside_animation();
