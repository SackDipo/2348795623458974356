#include "Prediction/Pridiction.h"
#include "../includes.h"
...
/* note ev0j1ix:
* oh yes, how cool is it to use a cocky APD on a zero rekorde 
*/
    if (!pPreviousRecord)
        return;
    ...
    for (int i = 0; i <= pRecord->m_iChoked; ++i) {
        ...
        if (!pRecord->m_bDidShot) {
            auto vecEyeAngles = math::Interpolate(pPreviousRecord->m_angEyeAngles, pRecord->m_angEyeAngles, pRecord->m_iChoked);
            m_pEntity->m_angEyeAngles().y = vecEyeAngles.y;
        }

        m_pEntity->m_flDuckAmount() = math::Interpolate(pPreviousRecord->m_flDuck, pRecord->m_flDuck, pRecord->m_iChoked);
...
void Animations::UpdatePlayer(Player* pEntity) {
    ...
    g_csgo.m_cl->m_bIsHLTV() = true;
    pEntity->UpdateClientSideAnimation();
    g_csgo.m_cl->m_bIsHLTV() = false;
...
void Animations::UpdateLocalAnimations() {
	...
	state->m_flLastUpdateIncrement = std::fmaxf(g_csgo.m_globals->m_curtime - state->m_flLastUpdateTime, 0.f);
	...
	if (g_cl.m_animate)
	{
		...
		if (g_csgo.m_cl->iChokedCommands == NULL) {
			g_cl.m_rotation.y = state->m_abs_yaw;
			g_cl.m_local->GetPoseParameters(g_cl.m_real_poses);
		}

		// remove model balance adjust animation (eye candy).
		g_cl.m_fake_layers[3].m_weight = FLT_EPSILON;
		g_cl.m_fake_layers[3].m_cycle = FLT_EPSILON;

		// remove model sway.
		g_cl.m_real_layers[12].m_weight = FLT_EPSILON;
	...
	// build bones.
	GenerateMatrix(g_cl.m_local, g_cl.m_real_matrix, BONE_USED_BY_ANYTHING, g_cl.m_local->m_flSimulationTime());
}
...
void Animations::UpdateFakeAnimations() {
	...
	else
	{
		...
		// allow re-animating in the same frame.
		if (g_cl.m_fake_state->m_nLastUpdateFrame >= g_csgo.m_globals->m_frame)
			g_cl.m_fake_state->m_nLastUpdateFrame = g_csgo.m_globals->m_frame - 1;

		// update anim update delta as server would.
		g_cl.m_fake_state->m_flLastUpdateIncrement = std::fmaxf(g_csgo.m_globals->m_curtime - g_cl.m_fake_state->m_flLastUpdateTime, 0.f);
		...
		if (g_cl.m_update_fake)
		{
			...
			// remove model sway.
			g_cl.m_fake_layers[12].m_weight = FLT_EPSILON;
		...
		g_cl.m_local->SetAbsAngles(ang_t(0.f, g_cl.m_fake_state->m_abs_yaw, 0.f));

		// generate a fake matrix.
		GenerateMatrix(g_cl.m_local, g_cl.m_fake_matrix, BONE_USED_BY_ANYTHING, g_cl.m_local->m_flSimulationTime());
