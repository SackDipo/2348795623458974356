...
float antiaim::get_yaw(CUserCmd* m_pcmd)
{
	...
	else
	{
		...
		case 2:
		{
			...
			{
				auto start_angle = (float)vars.antiaim.type[type].range * -60.f;
				auto end_angle = (float)vars.antiaim.type[type].range * 60.f;
	...
	if (should_break_lby(m_pcmd, lby_type))
	{
		...
		if (lby_type != 2 || sway_counter > 1)
		{
			if (desync_angle > 60.0f)
				yaw -= 179.0f;
			else
				yaw += 179.0f;
		}
